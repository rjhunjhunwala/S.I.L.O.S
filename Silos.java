...
public class Silos {
	...
	public static void main(String[] args) {
		...
		int[][] program = compile(interactive ? getStringFromSTDIN("FileName?",sc) : args[0]);
		int arg_index = 1;
		int ptr = 0;
...
//for(String s:texts)System.out.println(s);
		while (ptr < length) {
			try {
				tokens = program[ptr];//System.out.println(ptr+" "+(tokens[0]>>8));
		...
		try {
			...
			for(int i=0;i<tokens.size();i++){
				...
				if(instruction.equals("draw")){
					...
					program.add(new int[]{
						Silos.DRAW+(shape<<4)+(mode4<<3)+(mode3<<2)+(mode2<<1)+(mode1),
						arg1, arg2, arg3, arg4});
					continue;
				}
